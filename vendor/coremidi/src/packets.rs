...
impl Packet {
    ...
    pub fn data(&self) -> &[u8] {
        unsafe {
            let data_ptr = std::ptr::addr_of!(self.0.data).cast::<u8>();
...
impl PacketBuffer {
    ...
    fn aligned_bytes_len(&self) -> usize {
        if self.as_ref().is_empty() {
            ...
            let current_packet_data_ptr = unsafe { std::ptr::addr_of!((*current_packet_ptr).data).cast::<u8>() };
            let current_packet_length = unsafe { std::ptr::addr_of!((*current_packet_ptr).length).read_unaligned() };
            let data_offset = current_packet_data_ptr as usize - storage_start_ptr as usize;
            let data_len = current_packet_length as usize;
